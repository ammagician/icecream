...
export default {
  ...
  data() {
    ...
    return {
      loginForm: {
        username: localStorage.getItem("username") || 'admin1',
        password: '',
      ...
      loginRules: {
        ...
        tenant: [{ required: true, trigger: 'blur'}]
      },
      loading: false,
  ...
  methods: {
    ...
    handleLogin() {
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          this.loading = true
          const param = { ...this.loginForm, password: MD5(this.loginForm.password) }
          this.$store.dispatch('user/login', param).then(() => {
            localStorage.setItem("username", param.username)
            localStorage.setItem("tenantId", param.tenant)

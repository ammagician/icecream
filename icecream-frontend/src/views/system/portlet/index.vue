<template>
  <div class="app-container">
    ...
    <el-dialog
      ...
      :title="portlet.id ? '编辑' : '新增'"
      width="500px"
      size="small"
    >
      <el-form :model="portlet" :rules="rules" label-width="80px" size="small">
        <el-form-item label="名称" prop="name">
          <el-input v-model="portlet.name" placeholder="名称" />
        </el-form-item>
...
import { queryList, save, del } from '@/api/sys/portlet'
...
export default {
  ...
  methods: {
    ...
    handleConfirm() {
      save(this.portlet).then(res => {
        if (res.success) {
          this.queryList()
          this.dialogVisible = false
        } else {
          Message({
            message: (this.portlet.id ? '更新' : '新增') + '失败',
            type: 'error',
            duration: 3 * 1000
          })
        }
